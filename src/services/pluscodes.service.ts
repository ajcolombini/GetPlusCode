...
export class CodeArea {
  ...
  constructor(public latitudeLo: number, public longitudeLo: number, public latitudeHi: number, public longitudeHi: number, public codeLength: number | undefined) {
...
export default class OpenLocationCode {
  ...
  public static isValid(code: string): boolean {
    ...
    if (code.indexOf(OpenLocationCode.PADDING_CHARACTER_) > -1) {
      ...
      if (padMatch && (padMatch.length > 1 || padMatch[0].length % 2 === 1 || padMatch[0].length > OpenLocationCode.SEPARATOR_POSITION_ - 2)) {
        return false;
      }
  ...
  public static shorten(code: string, latitude: number, longitude: number): string {
    ...
    if ( codeArea?.codeLength??0 < OpenLocationCode.MIN_TRIMMABLE_CODE_LEN_) {
